...
echo "=== Exporting OTF/WOFF/WOFF2 from Glyphs ==="
...
-- Phase 3: Prepare OTF/WOFF/WOFF2 export dialogs on all documents
log "Preparing OTF/WOFF/WOFF2 exports..."
repeat with info in docInfos
    ...
    if not my hasCompletedPostScriptExport(projectRoot, outputPrefix) then
        with timeout of 7200 seconds
            tell application glyphsAppName
                open sourceAlias
            end tell
        ...
        delay 0.5
        log ("Preparing OTF/WOFF/WOFF2 export for " & normalizedSourcePath)
        my prepareExportOnFrontDocument("PostScript/CFF", ".otf", true, true, true, glyphsAppName, glyphsProcessName)
    else
...
log "Clicking Export Font on all OTF/WOFF/WOFF2 windows..."
my clickExportFontOnAllWindows(glyphsProcessName)

log "OTF/WOFF/WOFF2 exports kicked off."
APPLESCRIPT

echo "=== OTF/WOFF/WOFF2 export kicked off ==="
